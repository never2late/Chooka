...
function MapBox() {
    ...
    this.startPosition = {x: 150, y: 70, z: 0};
}
...
MapBox.prototype.setBox = function() {
    var mapLeft, mapTop;

    if (isPortrait == true) {
        mapLeft = (canvasHeight - mapSize) / 2;
        mapTop = (canvasWidth - mapSize) / 2;
    } else {
        mapLeft = (canvasWidth - mapSize) / 2;
        mapTop = (canvasHeight - mapSize) / 2;
    }

    this.box = {
        left: mapLeft,
        right: mapLeft + mapSize,
        top: mapTop,
        bottom: mapTop + mapSize
    };
};

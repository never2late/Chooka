			...
			switch ( mesh.objType ) {
				...
				case objType.kakaoBox:
					if (kakaoBox.getState() == boxState.normal) {
						...
						kakaoBox.setBounceTime(getTime());
					}
				break;
				...
				case objType.facebookBox:
					if (facebookBox.getState() == boxState.normal) {
						...
						facebookBox.setBounceTime(getTime());
				...
				case objType.twitterBox:
					if (twitterBox.getState() == boxState.normal) { 
						...
						twitterBox.setBounceTime(getTime());
					}
				break;

				case objType.mapBox:
					if (mapBox.getState() == boxState.normal) {mapBox.callNaverMapLink();
						mapBox.setState(boxState.bouncingUp);
						mapBox.setBounceTime(getTime());
